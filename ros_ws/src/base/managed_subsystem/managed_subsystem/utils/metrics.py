"""This file was taken from the original SynDrone paper implementation.
...
class Metrics:
    ...
    def __genterate_cm__(self, pred: torch.Tensor, gt: torch.Tensor):  #       preds
        mask = (gt >= 0) & (gt < self.num_classes).to(pred.device)  #     +-------
        combinations = (
            self.num_classes * gt[mask] + pred[mask]
        )  # 0 <= comb <= num_classes^2-1    #   l | . . .
    ...
    def add_sample(self, pred, gt):
        ...
        self.confusion_matrix += self.__genterate_cm__(
            pred, gt
        )  # labels along rows, predictions along columns
    ...
    def IoU(self):
        ...
        return torch.diagonal(self.confusion_matrix) / (
            ...
            - torch.diagonal(self.confusion_matrix)
        )
    ...
    def __str__(self):
        ...
        for i, n in enumerate(self.name_classes):
            ...
            if self.log_colors:
                cpa, cpp, ciou = (
                    self.get_color(npa, mpa, spa),
                    self.get_color(npp, mpp, spp),
                    self.get_color(niou, miou, siou),
                )
                tpa, tpp, tiou = (
                    self.color_tuple(npa, cpa),
                    self.color_tuple(npp, cpp),
                    self.color_tuple(niou, ciou),
                )
                out += (
                    "  "
                    + n
                    + " " * (16 - len(n))
                    + "\t %s%.1f%s\t %s%.1f%s\t %s%.1f%s\n" % (*tpa, *tpp, *tiou)
                )
            else:
